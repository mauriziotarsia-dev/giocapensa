...
- **Il primo suono della pagina:** i browser tengono l'audio addormentato finché non c'è un tocco, e il risveglio è asincrono. `sound.js` e il motore di Via libera, se il contesto non è ancora `running`, chiamano `resume()` e suonano l'effetto nel `then`: programmarlo subito lo farebbe cadere nel passato e uscirebbe muto (era il bug del «primo livello senza audio»).
